# `press` CLI Conventions — Requirements

- **Status:** Implemented (PRs #378–#380; naming per [ADR 0001](../adr/0001-app-short-name-press.md))
- **Type:** Design / requirements spec
- **Created:** 2026-06-09
- **Applies to:** the template-press CLI (`press`)
...
`template-press` CLI. App short name: **`press`**. Environment variable
prefix: **`PRESS`**.
...
- **App name (namespace):** `press`
- **Env prefix:** `PRESS` (every flag also resolves from `PRESS_*`)
...
- **R1.2** Click is configured with `auto_envvar_prefix="PRESS"` so every flag
  automatically resolves from a `PRESS_*` environment variable. Env wiring is
  not hand-maintained.
...
- **R2.2** All **logs, diagnostics, and human messaging** are written to
  **stderr** only.
- **R2.3** The two streams are never mixed, so `press ... | jq` and similar
...
- **R4.3** Shell redirection (`press ... > file`) remains fully supported; the
...
- **R6.2** Config file path: `$XDG_CONFIG_HOME/press/press_config.toml`
  (default `~/.config/press/press_config.toml`). `$XDG_CONFIG_HOME` is honored
  when set (not hardcoded to `~/.config`).
- **R6.3** Optional layered discovery, each layer overriding the previous:
  1. system: `$XDG_CONFIG_DIRS/press/press_config.toml`
  2. user: `$XDG_CONFIG_HOME/press/press_config.toml`
  3. project-local: `./press_config.toml` (or `./.press_config.toml`)
- **R6.4** `--config PATH` (env `PRESS_CONFIG`) overrides discovery entirely.
...
- **R8.2** The token is supplied via `--token` flag or the `PRESS_TOKEN`
...
- **R10.4** `--log-level {debug,info,warning,error,critical}` (env
  `PRESS_LOG_LEVEL`) is an explicit override of the console level.
...
- **R11.1** The file sink is enabled by `--log-file [PATH]` (env
  `PRESS_LOG_FILE`) or by setting `logging.file` in config.
- **R11.2** When enabled without an explicit path, the default location is
  `$XDG_STATE_HOME/press/press.log` (default `~/.local/state/press/press.log`).
...
- **R11.4** The file sink has its own level (`logging.file_level`, env
  `PRESS_LOG_FILE` companion / config), defaulting to `DEBUG`, independent of
  the console level.
- **R11.5** File sink format is controlled by `logging.format` (env
  `PRESS_LOG_FORMAT`): `json` (JSONL, recommended for the file) or `text`.
...
- **R12.1** All env vars are prefixed `PRESS_`. The full set:

| Variable          | Controls                                   | Equivalent flag        |
|-------------------|--------------------------------------------|------------------------|
| `PRESS_CONFIG`     | Path to config file (overrides discovery)  | `--config`             |
| `PRESS_OUTPUT`     | Output format (`text` \| `json`)           | `--output` / `-o`      |
| `PRESS_COLOR`      | Color mode (`auto`\|`always`\|`never`)     | `--no-color`           |
| `PRESS_LOG_LEVEL`  | Console log level                          | `--log-level`/`-v`/`-q`|
| `PRESS_LOG_FILE`   | Log file path (presence enables file sink) | `--log-file`           |
| `PRESS_LOG_FORMAT` | File sink format (`text` \| `json`)        | (config only)          |
| `PRESS_TOKEN`      | Auth token (secret)                        | `--token`              |
...
## Appendix A — Example `press_config.toml`

```toml
# $XDG_CONFIG_HOME/press/press_config.toml

[output]
format = "text"        # text | json        (PRESS_OUTPUT)
color  = "auto"        # auto | always | never

[logging]
level      = "warning" # console level       (PRESS_LOG_LEVEL)
file       = ""        # empty = off; path enables file sink (PRESS_LOG_FILE)
file_level = "debug"   # file sink level, independent of console
format     = "text"    # text | json (JSONL) for the file sink (PRESS_LOG_FORMAT)
```
...
| Config file    | `$XDG_CONFIG_HOME/press/press_config.toml`            |
| Console logging| on, `WARNING`, to stderr                            |
| File logging   | off                                                 |
| File log path  | `$XDG_STATE_HOME/press/press.log` (when enabled)      |
| File log format| JSONL recommended                                   |
| Precedence     | flag → env → project → user → system → default      |

## Appendix D — Migration notes for `template-press`
...
5. Keep the token in an env var (current `PY_TOKEN` → `PRESS_TOKEN`).
