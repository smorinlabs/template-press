...
- **G5++ (clean/smudge residual):** `git_hardening_args()`'s `-c` flags do NOT and cannot neutralize a repo-local `.git/config` `[filter "<name>"] clean=<cmd>` (filter drivers are arbitrarily named, no wildcard `-c` disables one) — a stat-mismatched tracked file on a hostile target can still trigger it via a working-tree read (`git status`); prefer index/object reads (`git ls-files` / `git ls-tree` / `git cat-file` / `git write-tree`) over `git status` in Tasks 1/3/12 wiring where feasible.
