# 0001. App short name `press` (hard rename from `pylb`)
...
- **Related:** [design 0001](../design/0001-press-cli-conventions.md) §0–§1; ADR 0002

## Context

The modern noun-verb CLI shipped as `pylb` with a `PY_TOKEN` env var, while
the conventions spec (design 0001) standardized on the short name **`press`**
with a **`PRESS_*`** env prefix. The name is load-bearing: it is the console
...
Rename to `press`/`PRESS_*` everywhere, as a **hard rename** — no `pylb`
...
was **removed in a later cleanup**, leaving `press` as the sole console script.)
