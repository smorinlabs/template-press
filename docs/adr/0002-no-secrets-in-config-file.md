...
- **Related:** [design 0001](../design/0001-press-cli-conventions.md) §8 (R8)
...
secrets live in the `--token` flag or `$PRESS_TOKEN` env var only: config
...
The token resolves from `--token` then `$PRESS_TOKEN` — **never** from a
...
- Setup docs must say "export PRESS_TOKEN" instead of "run config set".
