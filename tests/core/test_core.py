...
from template_press.core import (
    Config,
    ConfigPath,
    Project,
    ProjectList,
...
from template_press.core.logging import LogFormat, configure_logging, get_logger
from template_press.core.settings import writable_keys
...
def test_token_never_read_from_file(tmp_path, monkeypatch):
    ...
    cfg_file = tmp_path / "press_config.toml"
...
def test_load_config_missing(monkeypatch):
    monkeypatch.delenv(TOKEN_ENV_VAR, raising=False)
    cfg = load_config(config_file="/nonexistent/path/press_config.toml")
...
def test_settings_defaults_when_no_file(monkeypatch):
    monkeypatch.delenv(TOKEN_ENV_VAR, raising=False)
    cfg = load_config(config_file="/nonexistent/press_config.toml")
...
def test_config_file_naming_under_xdg(tmp_path, monkeypatch):
    ...
    assert paths.config_file() == tmp_path / "press" / "press_config.toml"


def test_database_file_naming_under_xdg(tmp_path, monkeypatch):
    monkeypatch.setenv("XDG_DATA_HOME", str(tmp_path))
    assert paths.database_file() == tmp_path / "press" / "press_db.db"


def test_state_file_naming_under_xdg(tmp_path, monkeypatch):
    monkeypatch.setenv("XDG_STATE_HOME", str(tmp_path))
    assert paths.state_file("history") == tmp_path / "press" / "press_history.log"
...
def test_log_level_vocabularies_stay_in_sync():
    ...
    from template_press.core.logging import LOG_LEVELS
...
def test_output_format_vocabularies_stay_in_sync():
    ...
    from template_press.cli.output import OutputMode
    from template_press.core.settings import OutputSettings
...
def test_write_leaves_no_temp_files(tmp_path):
    ...
    set_config_value(cfg_file, "output.color", "never")
    set_config_value(cfg_file, "logging.level", "info")
    assert [p.name for p in tmp_path.iterdir()] == ["press_config.toml"]
    if sys.platform != "win32":  # POSIX modes don't exist on windows
        assert (cfg_file.stat().st_mode & 0o777) == 0o600
