...
def test_git_listed_decodes_non_utf8_path_bytes_surrogateescape():
    ...
    """
    raw = b"? src/demo_widget/bad\xffname.txt\x00? README.md\x00"
    rels = [item[0] for item in parse_ls_files(raw)]
...
def test_gitlink_rels_decodes_non_utf8_path_bytes_surrogateescape():
    ...
    raw = (
        b"H 160000 " + b"0" * 40 + b" 0\tbad\xffsub\x00"
        b"H 100644 " + b"1" * 40 + b" 0\tREADME.md\x00"
    )
    rels = {
        rel.as_posix()
        for rel, _tracked, kind in parse_ls_files(raw)
        if kind == "gitlink"
    }
