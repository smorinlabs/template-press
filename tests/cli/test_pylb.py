"""Tests for the new noun-verb `press` CLI (via Click's CliRunner)."""
...
from template_press.cli.context import AppContext, maybe_show_first_run_hint
from template_press.cli.main import cli
from template_press.cli.output import OutputMode, Renderer
from template_press.core.config import Config
...
def test_completion_bash(runner):
    ...
    assert result.exit_code == 0
    assert "_PRESS_COMPLETE" in result.output
...
@pytest.fixture
def mock_service():
    with patch("template_press.cli.commands.projects.ProjectsService") as mock_cls:
        svc = Mock()
...
def test_doctor_human(runner, monkeypatch):
    monkeypatch.delenv("PRESS_TOKEN", raising=False)
    result = runner.invoke(cli, ["doctor", "--config", "/nope/press_config.toml"])
...
def test_doctor_json(runner, monkeypatch):
    monkeypatch.delenv("PRESS_TOKEN", raising=False)
    result = runner.invoke(
        cli, ["doctor", "--config", "/nope/press_config.toml", "--json"]
    )
    assert result.exit_code == 0
    payload = json.loads(result.output)
...
def test_config_get_setting(runner, tmp_path, monkeypatch):
    monkeypatch.delenv("PRESS_TOKEN", raising=False)
    cfg = tmp_path / "press_config.toml"
...
def test_config_set_env_var_resolution(runner, tmp_path, monkeypatch):
    # PRESS_OUTPUT resolves the --output format (R12); --json still overrides.
    monkeypatch.setenv("PRESS_OUTPUT", "json")
    monkeypatch.delenv("PRESS_TOKEN", raising=False)
    cfg = tmp_path / "press_config.toml"
    cfg.write_text('[logging]\nlevel = "info"\n')
    result = runner.invoke(
    ...
    assert result.exit_code == 0
    # PRESS_OUTPUT=json → output is parseable JSON without passing --json.
    payload = json.loads(result.output)
...
def test_output_format_resolves_from_config(runner, tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("PRESS_TOKEN", raising=False)
    monkeypatch.delenv("PRESS_OUTPUT", raising=False)
    cfg = tmp_path / "press_config.toml"
    cfg.write_text('[output]\nformat = "json"\n')
    result = runner.invoke(cli, ["config", "path", "--config", str(cfg)])
    assert result.exit_code == 0
...
def test_color_precedence_resolution(monkeypatch):
    from template_press.cli.context import _resolve_color
...
def test_log_file_flag_defaults_to_xdg_state(runner, tmp_path, monkeypatch):
    # R11.2: bare --log-file uses $XDG_STATE_HOME/press/press.log.
    monkeypatch.delenv("PRESS_TOKEN", raising=False)
    ...
    assert result.exit_code == 0
    assert (tmp_path / "press" / "press.log").exists()


def test_log_file_from_config(runner, tmp_path, monkeypatch):
    monkeypatch.delenv("PRESS_TOKEN", raising=False)
    monkeypatch.delenv("PRESS_LOG_FILE", raising=False)
...
def test_console_level_precedence():
    ...
    from template_press.cli.context import _resolve_console_level
...
def test_log_file_env_empty_enables_default(runner, tmp_path, monkeypatch):
    # R12: PRESENCE of PRESS_LOG_FILE enables the sink; empty value means
    # the default XDG state location.
    monkeypatch.delenv("PRESS_TOKEN", raising=False)
    monkeypatch.setenv("XDG_STATE_HOME", str(tmp_path))
    monkeypatch.setenv("PRESS_LOG_FILE", "")
    result = runner.invoke(
...
def test_config_get_reports_default_source(runner, tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("PRESS_TOKEN", raising=False)
    cfg = tmp_path / "press_config.toml"  # does not exist
    result = runner.invoke(
...
def test_auth_error_carries_error_code_and_hint(runner, monkeypatch):
    ...
    assert "PRESS_TOKEN" in payload["error"]["hint"]
...
def test_auth_error_text_shows_hint(runner, monkeypatch):
    ...
    result = runner.invoke(cli, ["projects", "list", "--config", "/nope/.env"])
    assert result.exit_code == 2
    assert "PRESS002" in result.output
...
def test_unexpected_error_writes_crash_log(runner, mock_service, tmp_path, monkeypatch):
    ...
    crash = tmp_path / "press" / "press_crash.log"
    assert crash.exists()
...
def test_unexpected_error_json_includes_traceback_path(
    ...
    assert result.exit_code == 4
    payload = json.loads(result.output)
    assert payload["error"]["error_code"] == "PRESS000"
    assert payload["error"]["traceback_path"].endswith("press_crash.log")
...
def test_first_run_hint_shown_once(monkeypatch, tmp_path, capsys):
    ...
    assert (tmp_path / "press" / "press_first_run.marker").exists()
...
def test_first_run_hint_json_mode_burns_nothing(monkeypatch, tmp_path, capsys):
    ...
    assert not (tmp_path / "press" / "press_first_run.marker").exists()
...
def test_doctor_bundle_text_summary(runner, monkeypatch):
    monkeypatch.setenv("PRESS_TOKEN", "t")
...
def test_unexpected_context_failure_follows_crash_contract(
    ...
    assert payload["error"]["error_code"] == "PRESS000"
    assert payload["error"]["traceback_path"].endswith("press_crash.log")
    assert (tmp_path / "press" / "press_crash.log").exists()
