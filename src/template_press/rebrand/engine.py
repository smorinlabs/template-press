...
def scan_paths(target: Path, rules: Rules) -> list[PathEntry]:
    ...
    Regenerable-lockfile exemption is deliberately keyed on
    ``DEFAULT_RULES.exclude_files`` (the tool's OWN built-in lockfile set),
    NOT ``rules.exclude_files`` — EMP-01: a target's ``extra_exclude_files``
    must not be able to hide content from the scan. Only a lockfile that is
    BOTH regenerated by this target's rules AND one of the tool's own
    exclude_files is exempt, because it is regenerated fresh after apply.
    ...
    """
    exempt_lockfiles = set(rules.regenerate) & DEFAULT_RULES.exclude_files
    out: list[PathEntry] = []
